...
class Setup extends \PrashantWP\Email_Tracker\Base {
    ...
    public function hook() {
	    ...
	    if ( emtr()->is__premium_only() ) {
	        ...
	        add_action( 'add_option_' . $option_name , array( $this, 'set_up_capability_to_roles_add_hook__premium_only' ), 10, 2 );
	        add_action( 'update_option_' . $option_name , array( $this, 'set_up_capability_to_roles_update_hook__premium_only' ), 10, 3 );
			add_action( 'init', function () {
				 if  ( ! wp_next_scheduled( 'email_tracker_delete_emails_after_days' ) ) {
					wp_schedule_event( time(), 'hourly', 'email_tracker_delete_emails_after_days', array(), false );
				}
				add_action( 'email_tracker_delete_emails_after_days', array( $this, 'email_tracker_delete_emails_after_days__premium_only' ) );
			} );
	    }
    ...
    public function set_up_capability_to_roles_add_hook__premium_only( $option, $value ) {
    ...
    public function set_up_capability_to_roles_update_hook__premium_only( $old_option_values, $new_option_values, $option ) {
        if ( ! current_user_can( 'manage_options' ) ) {
            return;
        }
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
        ...
        if ( emtr()->can_use_premium_code__premium_only() ) {
            $pro_asset_path = $asset_path . 'pro/';
            $pro_asset_url = $asset_url . 'pro/';
            $asset_file = include $pro_asset_path . 'index.asset.php';

            wp_enqueue_script(
                'emtr-email-list-pro',
                $pro_asset_url . 'index.js',
                array_merge(array('emtr-email-list'), $asset_file['dependencies']),
                $asset_file['version'],
                true
            );

            if ( file_exists( $pro_asset_path . 'index.css' ) ) {
                wp_enqueue_style(
                    'emtr-email-list-pro',
                    $pro_asset_url . 'index.css',
                    array(),
                    ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ? time() : '1.0.1',
                    'all'
                );
	...
	public function email_tracker_delete_emails_after_days__premium_only() {
		global $wpdb;

		$delete_emails_after_days = $this->factory->get( '\PrashantWP\Email_Tracker\Options' )->get( 'delete_emails_after_days', 30 );
		if ( $delete_emails_after_days <= 0 ) {
			return;
		}
    ...
    public function get_email_for_view( $request ) {
		...
		// For Duplicator like plugin send full path in scheduled package

	    if (  ! empty( $ret_data['attachments'] ) ) {
			$attachments_sep = ',\n';
		    $attachments = array_map( 'trim', explode( $attachments_sep, $ret_data['attachments'] ) );
		    $new_attachments_arr = array();
			$home_path_len = strlen( WP_CONTENT_DIR );

		    $upload_dir   = wp_upload_dir();
		    foreach ( $attachments as $attachment ) {
			    if ( 0 === stripos( $attachment, WP_CONTENT_DIR ) && 0 !== stripos( $attachment, $upload_dir['basedir'] ) ) {
				    $new_attachments_arr[] = substr( $attachment, $home_path_len  );
			    } else {
				    $new_attachments_arr[] = $attachment;
			    }
			}
